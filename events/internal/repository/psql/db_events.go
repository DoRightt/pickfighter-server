...
// TxCreateEvent creates a new event in the 'events' table and returns the event ID.
...
func (r *Repository) SearchEventsCount(ctx context.Context) (int32, error) {
	...
	filtered_events AS (
		SELECT
			...
			is_done
		FROM ranked_events
		WHERE (is_done = false AND rn <= $1) OR (is_done = true AND rn > (SELECT COUNT(*) FROM events.events WHERE is_done = true) - $1)
	)
...
// GetEventId retrieves the event ID associated with a specific fight from the fights table.
...
func (r *Repository) GetEventId(ctx context.Context, tx pgx.Tx, fightId int32) (int32, error) {
	q := "SELECT event_id FROM events.fights WHERE fight_id = $1"
...
// GetUndoneFightsCount retrieves the count of undone fights for a specific event from the fights table.
...
func (r *Repository) GetUndoneFightsCount(ctx context.Context, tx pgx.Tx, eventId int32) (int, error) {
	q := "SELECT COUNT(*) FROM events.fights WHERE event_id = $1 AND is_done = false"
	var count int
...
// SetEventDone updates the 'is_done' field of an event in the events table.
...
func (r *Repository) SetEventDone(ctx context.Context, tx pgx.Tx, eventId int32) error {
	q := "UPDATE events.events SET is_done = true WHERE event_id = $1"
