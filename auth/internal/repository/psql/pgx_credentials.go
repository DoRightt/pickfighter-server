...
func (r *Repository) TxNewAuthCredentials(ctx context.Context, tx pgx.Tx, uc model.UserCredentials) error {
	q := `INSERT INTO
		auth.user_credentials(user_id, email, password_hash, salt, token, token_type, token_expire, active)
...
// FindUserCredentials retrieves user credentials from the 'user_credentials' table based on the specified request parameters.
...
// ConfirmCredentialsToken updates the 'user_credentials' table to confirm user credentials based on the provided user ID.
...
func (r *Repository) ConfirmCredentialsToken(ctx context.Context, tx pgx.Tx, req model.UserCredentialsRequest) error {
	q := `UPDATE auth.user_credentials
...
// ResetPassword updates the 'user_credentials' table to reset a user's password based on the provided user credentials.
...
// UpdatePassword updates the password-related fields of a user in the 'user_credentials' table.
