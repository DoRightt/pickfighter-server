...
// It constructs a SQL query to count the number of records in the fighters table, applying
...
func (r *Repository) SearchFightersCount(ctx context.Context, req *model.FightersRequest) (int32, error) {
	q := `SELECT count(*) FROM fighters.fighters AS f`
...
// It constructs a SQL query to join the fighters and fighter_stats tables and applies
...
func (r *Repository) SearchFighters(ctx context.Context, req *model.FightersRequest) ([]*model.Fighter, error) {
	q := `SELECT f.fighter_id, f.name, f.nickname, f.division_id, f.status,
		...
		FROM fighters.fighters AS f
		LEFT JOIN fighters.fighter_stats AS fs ON f.fighter_id = fs.fighter_id`
