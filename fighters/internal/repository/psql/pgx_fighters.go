...
// The method constructs and executes a SQL query to select the fighter_id from the 'pf_fighters' table
...
func (r *Repository) FindFighter(ctx context.Context, req model.Fighter) (int32, error) {
	q := `SELECT fighter_id FROM pf_fighters WHERE name = $1 AND (fighter_url = $2 OR debut_timestamp = $3)`
	var fighterId int32

	err := r.GetPool().QueryRow(ctx, q, req.Name, req.FighterUrl, req.DebutTimestamp).Scan(&fighterId)
	if err != nil {
...
// to insert the provided fighter data into the 'public.pf_fighters' table. If a
...
func (r *Repository) CreateNewFighter(ctx context.Context, tx pgx.Tx, fighter model.Fighter) (int32, error) {
	qData := `INSERT INTO public.pf_fighters (
...
// to insert the provided statistics into the 'public.pf_fighter_stats' table. If a
// transaction (tx) is provided, the insertion is performed within that transaction;
...
func (r *Repository) CreateNewFighterStats(ctx context.Context, tx pgx.Tx, stats model.FighterStats) error {
	qStats := `INSERT INTO public.pf_fighter_stats (
...
// query to update the corresponding fields in the 'public.pf_fighters' table based
...
func (r *Repository) UpdateFighter(ctx context.Context, tx pgx.Tx, fighter model.Fighter) (int32, error) {
	qData := `UPDATE public.pf_fighters SET
...
// a SQL query to update the corresponding fields in the 'public.pf_fighter_stats'
...
func (r *Repository) UpdateFighterStats(ctx context.Context, tx pgx.Tx, stats model.FighterStats) error {
	qStats := `UPDATE public.pf_fighter_stats SET
