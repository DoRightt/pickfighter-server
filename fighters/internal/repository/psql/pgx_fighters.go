...
// The method constructs and executes a SQL query to select the fighter_id from the 'fighters' table
...
func (r *Repository) FindFighter(ctx context.Context, req model.Fighter) (int32, error) {
	q := `SELECT fighter_id FROM fighters.fighters WHERE name = $1 AND (fighter_url = $2 OR debut_timestamp = $3)`
	var fighterId int32
...
// to insert the provided fighter data into the 'fighters.fighters' table. If a
...
// to insert the provided statistics into the 'fighters.fighter_stats' table. If a
// transaction (tx) is provided, the insertion is performed within that transaction;
...
func (r *Repository) CreateNewFighterStats(ctx context.Context, tx pgx.Tx, stats model.FighterStats) error {
	qStats := `INSERT INTO fighters.fighter_stats (
...
// query to update the corresponding fields in the 'fighters.fighters' table based
...
// a SQL query to update the corresponding fields in the 'fighters.fighter_stats'
...
func (r *Repository) UpdateFighterStats(ctx context.Context, tx pgx.Tx, stats model.FighterStats) error {
	qStats := `UPDATE fighters.fighter_stats SET
